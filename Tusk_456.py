...
# также класс «Оргтехника», который будет базовым для классов-наследников. Эти классы — конкретные
...
# склад и передачу в определенное подразделение компании. Для хранения данных о наименовании и количестве
...
class StoreEquipment:
    ...
    def reception(self):
        ...
        if q == 'q' or q == "Q":
            ...
            return StoreEquipment.reception(self)


class Printer(StoreEquipment):
    def to_print(self):
        return f'Печатай {self.numb} раз'


class Scanner(StoreEquipment):
    def to_scan(self):
        return f'Сканируй {self.numb} раз'


class Copier(StoreEquipment):
    def to_copier(self):
        return f'Копируй  {self.numb} раз'


unit_1 = Printer('hp', 1000, 2, 12)
# unit_2 = Scanner('Canon', 2200, 3, 8)
# unit_3 = Copier('Xerox', 3500, 1, 10)
